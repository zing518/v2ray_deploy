...
web_camouflage() {
    ...
    git clone https://github.com/wangyangyangisme/3DCEList.git
    judge "web 站点伪装"
}
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/wangyangyangisme/v2ray_deploy/${github_branch}/v2ray-install-release.sh

    if [[ -f v2ray-install-release.sh ]]; then
        rm -rf $v2ray_systemd_file
        systemctl daemon-reload
        bash v2ray-install-release.sh --force
        judge "安装 V2ray"
    else
...
v2ray_conf_add_tls() {
    ...
    wget --no-check-certificate https://raw.githubusercontent.com/wangyangyangisme/v2ray_deploy/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/wangyangyangisme/v2ray_deploy/${github_branch}/ssl_update.sh"
...
basic_information() {
    {
        echo -e "${OK} ${GreenBG} V2ray+ws+tls+web 安装成功"
...
bbr_boost_sh() {
    ...
    wget -N --no-check-certificate "https://raw.githubusercontent.com/wangyangyangisme/Linux-NetSpeed/master/tcp.sh" && chmod +x tcp.sh && ./tcp.sh
}
mtproxy_sh() {
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/wangyangyangisme/v2ray_deploy/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/wangyangyangisme/v2ray_deploy/${github_branch}/install.sh
...
menu() {
    ...
    echo -e "\thttps://github.com/wangyangyangisme\n"
